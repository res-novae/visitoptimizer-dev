...
app.controller = (function() {
		                ...
		                settings = app.repository.loadSettings(function(settings) {
		                    ...
		                    if(settings.userId == 0) {
                                ...
                                app.repository.loadUser(settings.userId, function(user) {
                                    app.loggedUser = user;
                                    app.log("::Init:: Step 4 : Setting exist : loading success (id:"+app.loggedUser.id+")");
    ...
    $(document).bind("pagebeforeshow", function(e, data) {
        ...
        if($.mobile.activePage.attr('id') == 'vrn-pos-add-page'){
           controller.addPosForm();
        }
...
DetFen1'+roadmap[i].id_roadmap).bind('vclick',function (event){
    ...
    controller.showPosPanel = function() {
       ...
       //$.mobile.loadPage("pos_details.html",true);
        ...
        //$("[data-role=footer]").html(controller.getFooter('vrn-pos-page')).triggerAccess-Control-Allow-Origin:*('create');
        ...
        $.when(r1, r2).done(function(filter_list, pos_list) {
            ...
            for (var i=0;i<pos.length;i++){ 
                ...
                code += '<li data-icon="arrow-r2">'
                +'    <a href="pos_identity.html?sales_point_id='+pos[i].id_sales_point+'" data-transition="pop" data-rel="popup" data-inline="true">'
    ...
    controller.showPosIdentity = function(id_sales_point) {
        app.log("controller.showPosIdentity : "+id_sales_point , 'wip');
        ...
        r1.done(function(pos) {
            //alert('retour data : '+ pos.name);
            $('#vrn-pos-identity-title').val(pos.name);
            $('#vrn-pos-identity-address').val("<img src=\"./css/images/vrn/forma2_3.png\">"+ pos.street +", "+ pos.postal_code + " " + pos.city);
            $('#vrn-pos-identity-seller-name').val("<img src=\"./css/images/vrn/forma2_3.png\">"+pos.contact_name);
            ...
            $('#vrn-pos-textarea-description').val(pos.description);
            $('#vrn-pos-identity-page').trigger( "updatelayout" );
    ...
    // pos : add pos form
    controller.addPosForm = function() {  
        app.log("controller.addPosForm : " , 'wip');
      $("#vrn-pos-add-header").html(controller.getHeader()).trigger('create');
      $('#vrn-add-pos-enregistrer-button').bind('vclick',function (event){
          controller.addPosSave();
      });
    };
    
    
    // pos : add pos save
    controller.addPosSave = function() {
        app.log("controller.addPosSave : " , 'wip');
        var form_statut = "ok";
        var message = "";
        if($('#vrn-name-pdv').val() == ""){
        ...
        if(form_statut == "ok"){
            ...
            var data = [ $('#vrn-name-pdv').val(), $('#vrn-street').val(), $('#vrn-code-postal').val(), $('#vrn-city').val(), $('#vrn-contact-name').val(), $('#vrn-telephone').val(), $('#vrn-email').val(), 1 , user.id ];
            
            var r1 = app.repository.addPos(data);
    ...
    */
    ...
    controller.syncUpdateData = function(doneCallback) {
        ...
        controller.ajaxAsyncTask = "";
        app.log("cccc :"+app.loggedUser.id);
